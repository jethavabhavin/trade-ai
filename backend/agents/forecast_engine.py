...
try:
    from backend.schemas.forecast_schemas import PricePoint, ForecastPoint, MorningSignal
except ImportError:
    try:
...
class ForecastEngine:
    ...
    def generate_morning_signal(
        ...
        if history:
            for p in history:
                ...
                elif isinstance(p, dict) and 'close' in p:
                    closes.append(float(p['close']))
                else:
    ...
    def save_prediction_to_db(
        ...
        try:
            ...
            f_1d_dicts = [p.dict() if hasattr(p, 'dict') else (p.model_dump() if hasattr(p, 'model_dump') else p) for p in (forecast_1d or [])]
            f_7d_dicts = [p.dict() if hasattr(p, 'dict') else (p.model_dump() if hasattr(p, 'model_dump') else p) for p in (forecast_7d or [])]
    ...
    def run_timesfm_prediction(
        ...
        """
        try:
            from backend.agents.timesfm_service import timesfm_service
        except ImportError:
            try:
